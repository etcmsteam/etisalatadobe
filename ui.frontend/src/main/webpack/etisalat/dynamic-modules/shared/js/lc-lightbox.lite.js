...
(function ($) {
  ...
  var lcl = function (obj, settings) {
    ...
    var setup_code = function () {
      ...
      if (o.tn_hidden === true || (typeof o.tn_hidden == 'number' && ($(window).width() < o.tn_hidden || $(window).height() < o.tn_hidden))) {
        wrap_classes.push('lcl_tn_hidden');
      }

      // hide texts class
      if (o.txt_hidden === true || (typeof o.txt_hidden == 'number' && ($(window).width() < o.txt_hidden || $(window).height() < o.txt_hidden))) {
      ...
      if (o.cmd_position == 'inner' && o.ins_close_pos == 'corner') {
        css +=
          '#lcl_corner_close {' +
          'top: ' +
          (o.border_w + Math.ceil($('#lcl_corner_close').outerWidth() / 2)) * -1 +
          'px;' +
          'right: ' +
          (o.border_w + Math.ceil($('#lcl_corner_close').outerHeight() / 2)) * -1 +
          ';' +
          '}';

        // if no button is in inner cmd w/ corner close - hide bar (not on FS)
        if (!$('#lcl_nav_cmd > *:not(.lcl_close)').length) {
          css += '#lcl_wrap:not(.lcl_fullscreen_mode):not(.lcl_forced_outer_cmd) #lcl_nav_cmd {' + 'display: none;' + '}';
        }
      }
    ...
    var size_elem = function (el, flags, txt_und_sizes) {
      ...
      var side_txt =
        !$('.lcl_force_txt_over').length && !$('.lcl_hidden_txt').length && $.inArray(o.data_position, ['rside', 'lside']) !== -1 && elem_has_txt(el)
          ? $('#lcl_txt').outerWidth()
          : 0;
      ...
      if (typeof flags.inner_cmd_checked == 'undefined' && (o.cmd_position == 'inner' || el.force_outer_cmd) && inner_to_outer_cmd(el, w)) {
      ...
      lcl_size_n_show_timeout = setTimeout(function () {
        ...
        if ($('.lcl_first_sizing').length) {
          if (o.autoplay && v.elems.length > 1 && (o.carousel || v.elem_index < v.elems.length - 1)) {
            lcl_start_slideshow();
          }
        }
    ...
    var fs_img_manag = function (img_sizes) {
      ...
      // smart - fill only if is bigger than screen or same aspect ratio
        ...
        if (ratio_diff <= 1.15 && ratio_diff >= -1.15 && w_diff <= 350 && h_diff <= 350) {
          // fill
          $('.lcl_image_elem').css('background-size', 'cover');
        } else {
          // fit
              ...
              case 'vimeo':
                if (v.poster) {
                  ...
                  if (typeof lcl_ai_vars.vimeo_thumb_cache[v.src] == 'undefined') {
                    ...
                    $.getJSON('https://www.vimeo.com/api/v2/video/' + v.video_id + '.json?callback=?', { format: 'json' }, function (data) {
                      // need async
                      ...
                      $('.lcl_tn_inner li[rel=' + i + ']').attr(
                        'style',
                        $('.lcl_tn_inner li[rel=' + i + ']').attr('style') + " background-image: url('" + data[0].thumbnail_large + "');",
                      );
                    });
                  } else {
    ...
    $.fn.lcl_smoothscroll = function (ratio, duration, ignoreX, ignoreY) {
      ...
      $subj.mouseup(function (m) {
        ...
        if (scrollDiffY < 3 && scrollDiffY > -3 && scrollDiffX < 3 && scrollDiffX > -3) {
    ...
    var lcl_iframe_click = function () {
      ...
      lcl_iframe_click_intval = setInterval(function () {
        ...
        if (
          ...
          ($('.lcl_youtube_elem').length || $('.lcl_vimeo_elem').length || $('.lcl_dailymotion_elem').length)
        ) {
          lcl_stop_slideshow();
          $('#lcl_wrap').addClass('lcl_playing_video');
    ...
    $(document).on('click', '.lcl_tn_prev:not(.lcl_tn_disabled_arr)', function (e) {
      ...
      $('.lcl_tn_inner')
        .stop(true)
        .animate({ scrollLeft: $('.lcl_tn_inner').scrollLeft() - lcl_curr_opts.thumbs_w - 10 }, 300, 'linear', function () {
          $('.lcl_tn_inner').trigger('lcl_smoothscroll_end');
        });
    });
    ...
    lcl_stop_slideshow = function () {
      ...
      $('#lcl_progressbar')
        .stop(true)
        .animate({ marginTop: $('#lcl_progressbar').height() * -3 }, 300, function () {
          $(this).remove();
        });
