        ...
        return function () {
            ...
            $(document).ready(function () {
              ...
              var keyupSearchFunc = debounce(function(event) {
                if (event.isComposing || event.keyCode === 229) { //keycode 229 for Android browser fix to prevent placeholder value
                  return;
                }
                ...
                if(searchText.length > 2){
                  ...
                  //doc.querySelectorAll('.search-not-found-4-0')[instannceNum].classList.add('d-none');
                }
              }, 500);
              doc.querySelectorAll('form[name=searchForm]')[instannceNum].addEventListener('submit', function(e) {
              ...
              var searchTerm = function(searchText){
                ...
                var doc = document;
                var loadertemplate = '<div class="search-result-loader-4-0 sr-left">'+
                                        '<div class="ph-item">'+
              ...
              var guidedSearch = function(searchText, target){
                var loadertemplate = '<div class="search-result-loader-4-0 sr-right">'+
                                        //item
                                       '<div class="ph-item">'+
                                       '<div class="row">'+
              ...
              var checkType = function(item, required){
                  if(item.attributes['record.source'][0]==="ProductCatalog"){
                    if(item.attributes['sku.type'] !== undefined && item.attributes['sku.type'][0] === "mobilePlanSku" && item.attributes['sku.productType'] !== undefined && item.attributes['sku.productType'][0] === "MOBILE_PREPAID"){
                      return required === "class" ? "prepaid" : "PREPAID PLANS";
                    }
                    if(item.attributes['sku.type'] !== undefined && item.attributes['sku.type'][0] === "mobilePlanSku" && item.attributes['sku.productType'] !== undefined && item.attributes['sku.productType'][0] === "MOBILE_POSTPAID"){
              ...
              var getURL = function(item){
                  ...
                  if(item.attributes['record.source'][0]==="ProductCatalog"){
                    if(item.attributes['sku.type'] !== undefined && item.attributes['sku.type'][0] === "mobilePlanSku" && item.attributes['sku.productType'] !== undefined && item.attributes['sku.productType'][0].toLowerCase() === "mobile_prepaid"){
                      return deviceConfigUrl.prepaid+item.attributes['product.repositoryId'][0];
                    }
                    if(item.attributes['sku.type'] !== undefined && item.attributes['sku.type'][0] === "mobilePlanSku" && item.attributes['sku.productType'] !== undefined && item.attributes['sku.productType'][0].toLowerCase() === "mobile_postpaid"){
              ...
              var renderTabItems = function(json, statusText, xhr){
                    ...
                    if (json.success === true && statusText === "success" && json.response !== undefined) {
                      ...
                      for(var i=0; i < tabsBodyItemData.length && i < 3; i++){
                                  if(tabsBodyItemData[i].attributes['record.source'][0].toLowerCase() === "support" || tabsBodyItemData[i].attributes['record.source'][0].toLowerCase()==="web"){
                                    ...
                                    tabsBodyItemHTML +=     '</a>'+
                                                            '<div class="action-wrapper">'+
                                                                '<a href="'+tabsBodyItemData[i].attributes['record.id'][0]+'" class="media-action">'+checkType(tabsBodyItemData[i],"name") +'</a>'+
                                                            '</div>'+
                                                        '</div>';
                                  }else{
                                    ...
                                    tabsBodyItemHTML +=     '<div class="action-wrapper">';
                                                                if(tabsBodyItemData[i].attributes['product.templateUrl'] !== undefined){
                                    tabsBodyItemHTML +=           '<a href="'+tabsBodyItemData[i].attributes['product.templateUrl'][0]+'" class="media-action">'+checkType(tabsBodyItemData[i],"name") +'</a>';
                                                                }
                                                                else{
                                    tabsBodyItemHTML +=           '<a href="'+getURL(tabsBodyItemData[i])+'" class="media-action">'+checkType(tabsBodyItemData[i],"name") +'</a>';
                                                                }

                                    tabsBodyItemHTML +=    '</div>'+
                                                        '</div>';
                                  }

                      }
                              if(tabsBodyItemData.length >= 3){
                                  tabsBodyItemHTML += '<div class="col-xs-12 mt-30 mb-20 text-center">'+
                                                        '<a href="'+resultPageURL+'" class="btn-text btn-green no-arrow">'+
                                                              viewAll+
                                                        '</a>'+
                                                      '</div>';
                              }
                      ...
                      tabsBodyItemHTML +=  '<div class="results-media">'+
                                              '<div class="media-body">'+
                                                '<div class="media-description">No Record Found</div>'+
                                              '</div>'+
                                          '</div>';
                      document.getElementById(guidedSearchWraper).innerHTML = tabsBodyItemHTML;
                      return true;
                ...
                var submitSuccessResponse = function(json, statusText, xhr) {
                    ...
                    if (json.success === true && statusText === "success" && json.response.contents[0].autoSuggest[0].dimensionSearchGroups.length > 0) {
                      ...
                      for(var i=0; i<tabHeadData.length && i<5; i++){
                        tabsHeadHTML +='<a data-toggle="collapse" class="'+ (i > 0 ? "collapsed":"") +'" data-parent="#getMore'+instannceNum+'" href="#collapse1'+i+'">  '+tabHeadData[i].label +' </a>'
                        tabsBodyHTML += '<div id="collapse1'+i+'" class="panel-collapse collapse fade '+ (i > 0 ? "":"in") +'"> <div class="desktop-body">'+i+' </div> </div>'
                      }
                      doc.querySelectorAll('.getMore')[instannceNum].querySelector('.desktop-head').innerHTML = tabsHeadHTML;
                      doc.querySelectorAll('.tabsPanes-wrapper')[instannceNum].innerHTML =   '<div class="tabs-expansion-title-wrap"> <h4>'+topResult+'</h4> </div>'+ tabsBodyHTML;
                      doc.querySelectorAll('.search-items-wrapper')[instannceNum].classList.add('d-none');
                      ...
                      $(".tabs-expansion-section .desktop-head a").off('click').on("click", function (e) {
                        $(".tabs-expansion-section .desktop-head a").addClass()
                        var attr = $(this).attr("href");
                        var text = $(this).text().trim();
                        var divRef = $(this)
                          .parents(".tabs-expansion-section")
                          .find(".mobile-head")
                          .find(attr);
                ...
                var submitErrorResponse = function(jqXHR, textStatus, error) {
                  var errorText =
                    (jqXHR.responseJSON && jqXHR.responseJSON.message) || error;
                ...
                $('.search-link-cms .mega-menu-link').off('click').on('click', function (e) {
                console.log('click triggered for search button ');
                  ...
                  $('.backdrop-search-4-0').off('click').on('click', function (e) {
                    if(event.target.classList.contains('backdrop-search-4-0') || event.target.classList.contains('mega-menu-navbar')){
